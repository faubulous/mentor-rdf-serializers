...
export class JsonLdSerializer implements ISerializer {
    readonly syntax: RdfSyntaxType = RdfSyntax.JsonLd;
    ...
    private buildGraphContent(
        ...
        opts: Required<JsonLdSerializerOptions>
    ): JsonLdNode[] {
    ...
    private buildNode(
        ...
        if (subject.termType === 'NamedNode') {
            ...
            node['@type'] = this.compactIri(`${RDF_NAMESPACE}Statement`, opts);
            node[this.compactIri(`${RDF_NAMESPACE}subject`, opts)] = 
                this.buildValue(tt.subject, opts) as JsonLdValue;
            node[this.compactIri(`${RDF_NAMESPACE}predicate`, opts)] = 
                this.buildValue(tt.predicate, opts) as JsonLdValue;
            node[this.compactIri(`${RDF_NAMESPACE}object`, opts)] = 
                this.buildValue(tt.object, opts) as JsonLdValue;
        }
        ...
        for (const [predicateIri, propQuads] of properties) {
            ...
            if (opts.useTypeShorthand && predicateIri === RDF_TYPE) {
    ...
    private buildLiteralValue(literal: Literal, opts: Required<JsonLdSerializerOptions>): JsonLdValue {
        ...
        if (datatype === XSD_DOUBLE || datatype === XSD_DECIMAL) {
    ...
    private buildTripleTermValue(tripleTerm: TripleTerm, opts: Required<JsonLdSerializerOptions>): JsonLdNode {
        ...
        const result: JsonLdNode = {
            '@type': this.compactIri(`${RDF_NAMESPACE}Statement`, opts)
        };

        result[this.compactIri(`${RDF_NAMESPACE}subject`, opts)] = 
            this.buildValue(tripleTerm.subject, opts) as JsonLdValue;
        result[this.compactIri(`${RDF_NAMESPACE}predicate`, opts)] = 
            this.buildValue(tripleTerm.predicate, opts) as JsonLdValue;
        result[this.compactIri(`${RDF_NAMESPACE}object`, opts)] = 
            this.buildValue(tripleTerm.object, opts) as JsonLdValue;
